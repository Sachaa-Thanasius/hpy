...
if sys.implementation.name != 'cpython':
    msg = (
        'ERROR: Cannot install and/or update hpy on this python implementation:\n'
        f'    sys.implementation.name == {sys.implementation.name!r}\n\n'
    )
    if '_hpy_universal' in sys.builtin_module_names:
        ...
        if hasattr(_hpy_universal, 'get_version'):
            ...
            msg += (
                f'This python implementation comes with its own version of hpy=={hpy_version}\n'
                '\n'
                'If you are trying to install hpy through pip, consider to put the\n'
                'following in your requirements.txt, to make sure that pip will NOT\n'
                'try to re-install it:\n'
                f'    hpy=={hpy_version}'
            )
        else:
            msg += (
                'This python implementation comes with its own version of hpy,\n'
                'but the exact version could not be determined.\n'
            )
    else:
        # this seems to be a python which does not support hpy
        msg += (
            'This python implementation does not seem to support hpy:\n'
            '(built-in module _hpy_universal not found).\n'
...
class build_clib_hpy(build_clib):
    ...
    def get_library_names(self) -> None:
